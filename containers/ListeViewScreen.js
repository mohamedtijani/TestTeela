...
export default class ListeViewScreen extends PureComponent {
	...
	_renderItem = ({ item }) => {
		return <Product product={item} direction={'row'}/>
	};

	_renderHeader = ({item}) => (
			<View style={{backgroundColor: '#ffff',justifyContent:'center', alignItems:'center', height: height / 11}}>
				<Text style={{fontSize: 20, fontWeight: 'bold'}}>Liste View</Text>
			</View>
);
